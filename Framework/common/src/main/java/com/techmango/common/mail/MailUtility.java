...
import com.techmango.common.util.CommonConstants;
...
public class MailUtility {
	...
	public static String ticketTemplateBuilder(Mail mail) {
		String messageContent = "";
		...
		try {
			...
			paramMap.put("ticketNumber", mail.getTicketNumber());
			messageContent = FreeMarkerTemplateUtils.processTemplateIntoString(template, paramMap);
		}
		catch(TemplateException templeateException) {
			throw new RuntimeException(templeateException);
		}
		catch(Exception e) {
			throw new RuntimeException(e);
		}
